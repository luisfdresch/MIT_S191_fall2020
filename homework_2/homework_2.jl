...
The optimization reduced 342 allocations (approx. 33%). I assume the optimization comes from removing the vcat function, that creates a new array form its arguments each time the line is executed. The alternative presented above does not use this function, therefore it creates one less matrix everytime the loop is executed
"""
...
684 allocations were avoided. The @view optimizetion prevents the creation of two other matrixes. In fact the loop does not need to create any new matrixes, only reallocate values between two existent matrixes
"""
...
# ╔═╡ 3d1335f2-ff7d-11ea-3239-a5934d4a1f71
eltype(energy(img)[1,1])
...
function greedy_seam(energies, starting_pixel::Int)
	pixels = Array{Int,1}(undef, size(energies)[1])
	pixels[1] = starting_pixel
	
	for i = 2:size(energies)[1]
		#range = pixels[i-1] == 1 ? (1:2) : (pixels[i-1] == size(energies)[2] ? (size(energies)[2]-1:size(energies)[2]) : (pixels[i-1]-1:pixels[i-1]+1))
		range =  max(pixels[i-1]-1,1):min(pixels[i-1]+1,size(energies)[2])
		pixels[i] = collect(Iterators.flatten(range))[findmin(energies[i, range])[2][1]]
	end
	return pixels
end
...
function least_energy(energies, i, j)
	my_range = max(1, j-1):min(size(energies)[2],j+1)
	if i == size(energies)[1]
		u = findmin([energies[i,r] for r in my_range])
		return (energies[i,j], my_range[u[2]]-j)
	end
	energy_sum, ind = least_energy(energies, i+1, j) 
	v = findmin([least_energy(energies,i+1,r) for r in my_range])
...
For every pixel $(1, j)$ the function least_energy is called recursivily for every pixel below itself and its neighbours, until it reaches the last row, so all the paths are evaluated even if that means that the energy will be calculated multiple times for a given pixel

The number of possible seams evaluated is approximately $O(n \cdot 3^m)$ for a $(m \times n)$ matrix.
"""
	...
	function memoized_least_energy(energies, i, j, memory)
		...
		elseif m == i
			...
			for (q,r) in enumerate(myrange)
				if haskey(memory, (i+1, r))
					tmp[q] = memory[(i+1, r)]
				else
					memory = memoized_least_energy(energies, i+1, r, memory)
					tmp[q] = memory[(i+1, r)]
				end
			end
...
function recursive_memoized_seam(energies, starting_pixel)
	memory = Dict{Tuple{Int,Int}, Float64}() # location => least energy.											# pass this every time you call memoized_least_energy.
	m, n = size(energies)
