...
# ╔═╡ f1e53838-0d45-11eb-0365-b9e91e9c7afa
using Statistics

# ╔═╡ b2aed3fa-0d8f-11eb-2953-4bff0c4b6d71
using Random
...
$p = 1$ means 100% probability of recovery
...
function frequencies(values)
	Dict([(i, sum(values .== i)/length(values)) for i in unique(values)])
end
...
function frequencies_plot_with_mean(data)
	...
	base = bar(frequencies(data))
	vline!(base, [mean(data)], label="mean = $(mean(data))")
...
md"""
p$(@bind p_interactive Slider(0.1:0.01:1, show_value = true))


N$(@bind N_interactive Slider(1:1_000:10_001, show_value = true))
"""

# ╔═╡ 86472b26-0d46-11eb-29bf-27db0a017db7
...
md"The distribution seems to have a geometric shape"

# ╔═╡ a9ae7c44-0d47-11eb-2270-f9fa2bec4e9f
begin
	data_test = [p_interactive*(1-p_interactive)^(i-1) for i = 1:100]
	base = bar(data_test)
	mean_test = sum(data_test.*collect(1:length(data_test)))/sum(data_test)
	vline!(base, [mean_test], label="$mean_test")
end
...
p_sweep = Dict([(i,mean(do_experiment(i,10_000))) for i = 0.001:0.001:1])

# ╔═╡ aa9709a8-0d85-11eb-0727-af00593b24cb
scatter(p_sweep, yaxis=:log)
...
# ╔═╡ fe9f8f36-0d86-11eb-1ff0-29ed19669c27
Int(R)
...
# ╔═╡ 3c04257c-0d88-11eb-1f71-2563755fbff8
...
# ╔═╡ e903e576-0d8f-11eb-2753-4db7744c2dbf
A= B =C = zeros(5)

# ╔═╡ f34e1b3c-0d8f-11eb-01ec-23f836fdcd08
B[2]=1

# ╔═╡ fa53ca44-0d8f-11eb-3a17-3d84daccd211
C
...
function sir_mean_plot(simulations::Vector{<:NamedTuple})
	...
	total = length(simulations)
	S_mean /= total
	I_mean /= total
	R_mean /= total
	p = plot()
	plot!(p, 1:T, S_mean, color = "lightblue", label = "Susceptible")
	plot!(p, 1:T, I_mean, color = "red", label = "Inefected")
	plot!(p, 1:T, R_mean, color = "green", label = "Recovered")
...
function sir_mean_error_plot(simulations::Vector{<:NamedTuple})
	...
	I_mean = mean(I, dims = 2)
	R_mean = mean(R, dims = 2)
	...
	p = plot()
	plot!(p, 1:T, S_mean, yerr = S_std, alpha= 0.4, color = "lightblue", label = "Susceptible")
	plot!(p, 1:T, I_mean, yerr = I_std, alpha= 0.4,color = "red", label = "Inefected")
	plot!(p, 1:T, R_mean, yerr = R_std, alpha= 0.4,color = "green", label = "Recovered")
	return p
end
...
# ╔═╡ 72e5a99c-0ef7-11eb-0cc8-170946d074be
function get_num_infected(agent::Agent)
	return agent.num_infected
end

# ╔═╡ 26e2978e-0435-11eb-0d61-25f552d2771e
function num_infected_plot(simulations::Vector{<:NamedTuple})
	T = length(first(simulations).S)
	total = length(simulations)
	temp = Dict()
	for sim in simulations
		temp = merge(+, temp, sim.F[T])
	end
	map!(x -> x/T, values(temp))
	p = bar(temp)
	return p
end
...
md"""
The maximum slope of the infection curve, the total agents infected, $R_0$"""
...
begin
	function interact!(agent::Agent, source::Agent, infection::InfectionRecovery)
		if is_infected(source) && is_susceptible(agent) && rand() < infection.p_infection 
			agent.status = I
...
function step!(agents::Vector{Agent}, infection::AbstractInfection)
	interact!(Random.shuffle(agents)[1:2]...,infection)
end
...
function simulation(N::Integer, T::Integer, infection::AbstractInfection)
	...
	I_counts = zeros(T)
	R_counts = zeros(T)
	freq_num_infected = Vector{Dict}(undef, T)
	for i = 1:T
		sweep!(agents, infection)
		S_counts[i] = count(is_susceptible.(agents))
		I_counts[i] = count(is_infected.(agents))
		R_counts[i] = N - S_counts[i] - I_counts[i]
		freq_num_infected[i] = frequencies(get_num_infected.(agents))
	end
		
	
	return (S = S_counts, I = I_counts, R = R_counts, F = freq_num_infected)
end
...
let
	p = plot()
	aux = zeros(length(simulations[1].I))
	for sim in simulations
		plot!(p, 1:1000, sim.I, alpha=.5, label=nothing)
		aux = aux .+ sim.I
	end
	plot!(p, 1:1000, aux/length(simulations) , alpha = 1, lw = 3, label = "mean", color = "red")
	p
end
...
md"""
$p_{infection}$ $(@bind p_infection_sim Slider(0:0.01:1, show_value = true))

$p_{recovery}$ $(@bind p_recovery_sim Slider(0:0.001:0.5, show_value = true))
"""

# ╔═╡ 1c26648e-0efc-11eb-2b4c-afd25db0dc52
interactive_simulations_data = repeat_simulations(100, 1000, InfectionRecovery(p_infection_sim, p_recovery_sim), 20)

# ╔═╡ cdd43212-0e23-11eb-32b2-1f24df4d1057
sir_mean_plot(interactive_simulations_data
)

# ╔═╡ 13352af6-0ef5-11eb-1ec4-65413a2e37c9
sir_mean_error_plot(interactive_simulations_data)

# ╔═╡ 854e7e94-0efa-11eb-10a5-47c748c48280
num_infected_plot(interactive_simulations_data)

# ╔═╡ 1ac4b33a-0435-11eb-36f8-8f3f81ae7844
reinfection_simulations = repeat_simulations(100, 1000, Reinfection(p_infection_sim, p_recovery_sim), 20)

# ╔═╡ 998a55ac-0efc-11eb-3859-256113868277
let 
	p = plot()
	T = length(first(reinfection_simulations).S)
	aux = zeros(T)
	for sim in reinfection_simulations
		plot!(p, 1:T, sim.I, alpha = 0.5, label = nothing)
		aux = aux .+ sim.I
	end
	plot!(p, 1:T, aux/20, color = "red", lw = 3, alpha = 1, label = "mean")
	p
end
