...
# ╔═╡ 9c09d966-07da-11eb-19a2-7b88a33b3082
Pkg.status()
...
The following samples are from Wikipedia, but feel free to type in your own sample! You can also enter a sample of a different language, if tht language can be expressed in the Latin alphabet.
...
unused_letters = let
	alphabet[findall(isequal(0), sample_freqs)]
end
...
# ╔═╡ 8ff0ccd0-07de-11eb-2d36-cde89e822a47


# ╔═╡ 65c92cac-f930-11ea-20b1-6b8f45b3f262
double_letters = alphabet[[i for i in 1:length(alphabet)  if sample_freq_matrix[i,i] > 0 ] ]
...
# ╔═╡ 393fe980-07e2-11eb-1b84-dffcbee3c618
#" Sum of columns"
reshape(sum(sample_freq_matrix, dims = 1),27,)

# ╔═╡ 4a6b2936-07e2-11eb-37d4-dd81d7fcc25e
reshape(sum(sample_freq_matrix, dims = 2) ,27,)

# ╔═╡ 3ec6568e-07e3-11eb-1bea-a326b2d12081
sample_freqs
...
The sum of each row and columns is approximately the frequency of each letter, the difference is caused by the combination of letters so that the first and last pair of letters in the string are affected. This discrepancy is expected to decrease as the sample size increases.
...
function completions_cache(grams)
	...
	# your code here
	n = size(grams[1])[1]
	map(1:length(grams)-(n-1)) do i
		mykey = grams[i][1:n-1]
		if haskey(cache, mykey)
			cache[mykey] = [cache[mykey] grams[i][n]]
		else
			cache[mykey] = [grams[i][n]]
		end
	end
	cache
end
...
# ╔═╡ 1bee3796-07fc-11eb-0cf9-3f5b12926e0e
