...
ex_1_1 = md"""
If the square root is smaller than $a$, $x/a$ yields an answer bigger than the correct answer, and the opposite is true.

Let $c$ be the correct answer, so

$c = sqrt(x)$ and, 

$x = c^2$

Let $r$ be the ratio $x/a$, so

$r = \frac{x}{a} = \frac{c^2}{a}$

We can conclude tha if $a$ is equal to the correct answer, $r = c$. 

If $a < c \implies r > c$

Similarly, if $a > c \implies r < c$

So the correct answer is always between $a$ and $\frac{x}{a}$.
